<?php 
	$count_on_page = 20;
	$start_count_art = (is_null($_GET['page']))?0:((intval($_GET['page'])-1)*$count_on_page);
	$query = sprintf("SELECT SQL_CALC_FOUND_ROWS * FROM articles WHERE sub_category='%s' ORDER BY id_art DESC LIMIT %s, %s",$_GET['subcat'],$start_count_art, $count_on_page);
	...
	if (!empty($articles['id_art'])) {
		...
		if (intval($count['count'])%$count_on_page != 0){
			for ($i=0; $i<=intval($count['count'])/$count_on_page; $i++){
				if($i+1 == $curr)
				printf("<a class='curr_page' href='%s/%s/page%s'>%s</a>",$_GET['cat'], $_GET['subcat'], $i+1, $i+1);
				else
				printf("<a href='%s/%s/page%s'>%s</a>",$_GET['cat'],$_GET['subcat'], $i+1, $i+1);
			}
		}
		else{
			for ($i=0; $i<intval($count['count'])/$count_on_page; $i++){
